...
class MetafieldTest < Test::Unit::TestCase
  ...
  def test_create_metafield_for_a_blog
    ...
    metafield = blog.add_metafield(ShopifyAPI::Metafield.new(:namespace => "summaries", :key => "First Summary", :value => "Make commerce better", :type => "string"))

    assert_equal ActiveSupport::JSON.decode('{"metafield":{"namespace":"summaries","key":"First Summary","value":"Make commerce better","type":"string"}}'), ActiveSupport::JSON.decode(FakeWeb.last_request.body)
